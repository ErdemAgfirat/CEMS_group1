...
As shown in @fig-adc-conversion, the general block diagram of an ADC designed for converting low-pass signals consists of an anti-aliasing filter, a sampler, and a quantizer [@delarosa2013].

![A/D Conversion](figures/adc.drawio.svg){#fig-adc-conversion}

## Sampling

A set of key components is essential for converting an analog signal into a discrete-time representation. A fundamental aspect of this process is sampling, as it ensures the conversion of an analog signal into a discrete-time signal [@bajdechi2004systematic].
...
![Original spectrum](figures/spectrum.drawio.svg){#fig-Original-spectrum width=75%}

![Periodic repetition of the spectrum](figures/periodic_repitition_spectrum.drawio.svg){#fig-spectrum-repetition width=75%}
...
![Reconstruction by filtering with a low-pass filter](figures/ideal_fitler.drawio.svg){#fig-Reconstruction-by-filtering-with-a-low-pass-filter width=75%}
...
![Reconstruction of a sampled signal using sinc interpolation](figures/sinc_interpolation.svg){#fig-Reconstruction-of-a-sampled-signal-using-sinc-interpolation}
...
![Quantization characteristic [@goldenbaum2022grundlagen]](figures/quantization_characteristic-Seite-2.drawio.svg){#fig-Quantization-characteristic width=58%}
